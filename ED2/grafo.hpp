...
//Adicionar vertice
//https://stackoverflow.com/questions/2748036/c-expanding-an-array-with-malloc
//https://github.com/JadielJr/Grafos-/blob/master/grafo.cpp
...
Grafo *removerVertice(Grafo *g, int valor) {
    if(g->adj[valor].cab == NULL) {
        int novoTamanho = g->numVertice - 1;
        Grafo *aux = g;
        aux->adj = (Vertice*)malloc(sizeof(Vertice)*novoTamanho);
        int i = 0;
        int a = 0;
        while(a < novoTamanho) {
            ...
            aux->adj[a].cab = g->adj[i].cab;
            aux->adj[a].dado = g->adj[i].dado;
            a++;
            i++;
        }
        g->adj = aux->adj;
